...
def recognize_online(audio_filename, text_filename, language='en-US'):
    '''
    Recognize audio from 'audio_filename' with Google API and 
...
def main():
    if len(sys.argv) == 1:  # if parameter wasn't specified
        ...
        print('>>> python script_online_sr.py filename.wav')
        sys.exit()
    elif len(sys.argv) == 2:
    ...
    recognize_online(audio_filename, text_filename)
